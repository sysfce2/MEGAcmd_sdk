...
void MegaClient::initScStreamingParser()
{
    // Start of parsing
    scStreamingParser.addFilter("",
                                [this](JSON*)
                                {
                                    scStreamingParser.originalAC = actionpacketsCurrent;
                                    actionpacketsCurrent = false;

                                    // Start timer
                                    scStreamingParser.ccst =
                                        std::make_unique<CodeCounter::ScopeTimer>(
                                            performanceStats.scProcessingTime);
    ...
    scStreamingParser.addFilter("<",
                                [this](JSON*)
                                {
                                    assert(!scStreamingParser.nodeTreeIsChanging.owns_lock());
                                    scStreamingParser.nodeTreeIsChanging =
    ...
    scStreamingParser.addFilter(">",
                                [this](JSON*)
                                {
                                    if (scStreamingParser.nodeTreeIsChanging.owns_lock())
                                    {
                                        scStreamingParser.nodeTreeIsChanging.unlock();
                                    }
                                    return JSONSplitter::CallbackResult::SUCCESS;
                                });

    scStreamingParser.addFilter("{\"w",
                                [this](JSON* json)
                                {
    ...
    scStreamingParser.addFilter("{\"ir",
                                [this](JSON* json)
                                {
    ...
    scStreamingParser.addFilter("{\"sn",
                                [this](JSON* json)
                                {
                                    sc_storeSn(*json);
                                    return JSONSplitter::CallbackResult::SUCCESS;
    ...
    scStreamingParser.addFilter("{[a{\"a",
                                [this](JSON* json)
                                {
                                    scStreamingParser.actionName = json->getnameidvalue();
                                    return JSONSplitter::CallbackResult::SUCCESS;
                                });

    scStreamingParser.addFilter("{[a{\"i",
                                [this](JSON* json)
                                {
                                    string jsonSid;
                                    ...
                                    Base64::atob(jsonSid.c_str(), sid, sizeof(sid));

                                    scStreamingParser.isSelfOriginating =
                                        !memcmp(sid, sessionid, sizeof sessionid);

                                    return JSONSplitter::CallbackResult::SUCCESS;
                                });

    scStreamingParser.addFilter("{[a{\"st",
                                [this](JSON* json)
                                {
                                    json->storeobject(&scStreamingParser.seqTag);

                                    if (!sc_checkSequenceTag(scStreamingParser.seqTag))
                                    {
    ...
    scStreamingParser.addFilter(
        ...
        {
            // 'st' is not present
            if (scStreamingParser.seqTag.empty())
            {
                sc_checkActionPacketWithoutSt(scStreamingParser.actionName,
                                              scStreamingParser.lastAPDeletedNode.get());
            }

            if (scStreamingParser.actionName == 0)
            {
                scStreamingParser.lastAPDeletedNode.reset();
                return JSONSplitter::CallbackResult::SUCCESS;
            }

            sc_procActionPacketWithoutCommonTags(*json,
                                                 scStreamingParser.actionName,
                                                 scStreamingParser.isSelfOriginating,
                                                 scStreamingParser.lastAPDeletedNode);

            return JSONSplitter::ResultFromBool(json->leaveobject());
        });

    // End of action packets
    scStreamingParser.addFilter("{[a",
                                [this](JSON* json)
                                {
                                    scStreamingParser.clearActionPacketData();
    ...
    scStreamingParser.addFilter("{",
                                [&, this](JSON*)
                                {
                                    sc_procEoo(scStreamingParser.nodeTreeIsChanging,
                                               scStreamingParser.originalAC);
                                    return JSONSplitter::CallbackResult::SUCCESS;
                                });
}
